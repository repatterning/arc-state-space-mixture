...
class Foci:
    ...
    def __filtering(self, warnings: pd.DataFrame) -> pd.DataFrame:
        ...
        instances = warnings[['ending', 'warning_id']].drop_duplicates()
        instances.sort_values(by='ending', ascending=True, inplace=True)
        elements = instances.iloc[-1, :].squeeze()

        conditionals = ((warnings['warning_id'].isin([elements.warning_id])) &
